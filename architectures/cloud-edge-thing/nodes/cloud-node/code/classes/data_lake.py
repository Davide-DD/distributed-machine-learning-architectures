...
class DataLake:
	...
	def __init__(self):
		...
		data = [os.path.join(fileDir, 'TS2.txt'),os.path.join(fileDir, 'TS1.txt'),os.path.join(fileDir, 'TS3.txt'),os.path.join(fileDir, 'TS4.txt')]
		df = pd.DataFrame()
		for txt in data:
	...
	def add_data(self, data, outcome):
		for name in ['TS1', 'TS2', 'TS3', 'TS4']:
			filePath = os.path.join(fileDir, name + '.txt')
			f = open(filePath, "a")
			f.write(data[name])
			f.close()
		filePath = os.path.join(fileDir, 'profile.txt')
		f = open(filePath, "a")
		f.write(outcome)
		f.close()
