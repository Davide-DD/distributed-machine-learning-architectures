...
def create_node_instance(destination_path, node_type_path, instance_number, train_files=None, test_files=None):
...
for nt in get_node_types():
	...
	elif nt_basename == 'aggregator' or nt_basename == 'coordinator' or nt_basename == 'master-aggregator':
		for i in range(0, nodes_number):
			create_node_instance(destination_path, nt, i)
	elif nt_basename == 'thing':
		examples_number = int(input('How many records do you want to give to each node\'s dataset?\n'))
		for i in range(0, nodes_number):
			train_files, train_lines = get_examples(examples_number, selected_lines)
			create_node_instance(destination_path, nt, i, train_files)
			selected_lines += train_lines